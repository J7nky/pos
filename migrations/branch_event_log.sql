...
-- Unique constraint: one version per branch (enforced by application logic)
-- Note: We don't add DB constraint here to allow for concurrent inserts
-- Application must use sequence or atomic increment

-- RLS Policies
ALTER TABLE branch_event_log ENABLE ROW LEVEL SECURITY;

-- Policy: Users can only see events for their store
-- (Simplified - assumes user can access all branches in their store)
CREATE POLICY "Users can view events for their store"
  ON branch_event_log
  FOR SELECT
  USING (
    store_id IN (
...
-- Policy: Service role can insert events (for server-side triggers)
