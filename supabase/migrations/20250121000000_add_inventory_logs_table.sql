/*
  ...
  This migration adds a comprehensive inventory_logs table to track every single
  inventory item change, allowing transactions to reference specific inventory logs.

  Features:
  - Track every inventory item change (receive, sell, adjust, transfer)
  - Link transactions to specific inventory logs
  - Maintain audit trail for inventory movements
  - Support for FIFO tracking and cost calculations
*/
...
CREATE TABLE IF NOT EXISTS inventory_logs (
  ...
  quantity_change integer NOT NULL, -- positive for received, negative for sold/adjusted
  quantity_before integer NOT NULL,
  quantity_after integer NOT NULL,
  unit_price decimal(10,2), -- price at time of action
  total_value decimal(10,2), -- total value of the change
  currency text NOT NULL CHECK (currency IN ('USD', 'LBP')),
  reference_type text, -- 'sale', 'transaction', 'adjustment', 'transfer'
  reference_id uuid, -- ID of the related sale, transaction, etc.
  reference_description text, -- Human readable description
  notes text,
