...
class POSDatabase extends Dexie {
  ...
  constructor() {
    ...
    // Migration for version 52 - change bills.customer_id to bills.entity_id
    this.version(52).stores({
      ...
      // Bill management tables - UPDATED: customer_id changed to entity_id
      ...
      console.log('🔧 Running migration v52: Change bills.customer_id to bills.entity_id');
      console.log('   ✅ Migrating customer_id values to entity_id for existing bills');
      
      // Migrate existing customer_id values to entity_id
      return trans.table('bills').toCollection().modify((bill: any) => {
        // Copy customer_id to entity_id if entity_id doesn't exist
        if (bill.customer_id !== undefined && bill.customer_id !== null && bill.entity_id === undefined) {
          bill.entity_id = bill.customer_id;
        }
        // Remove customer_id field (Dexie will handle this automatically when schema changes)
        delete bill.customer_id;
      }).then(() => {
        console.log('   ✅ Migration completed: All bills now use entity_id');
      });
    });
  }
