// Arabic default translations
// Keys are structured by feature/page.component and UI element purpose
const ar = {
  app: {
    name: 'نظام نقاط بيع المنتجات',
    loading: 'جار التحميل...'
  },
  common: {
    recent: 'مؤخرًا',
    closed: 'مغلق',
    loading: 'جار التحميل...',
    active: 'مفعل',
    recentReceives: 'الاستلامات الأخيرة',
    transactions: 'المعاملات',
    actions: {
      cancel: 'إلغاء',
      save: 'حفظ',
      delete: 'حذف',
      edit: 'تعديل',
      add: 'إضافة',
      view: 'عرض',
      close: 'إغلاق',
      export: 'تصدير',
      refresh: 'تحديث',
      search: 'بحث',
      details: 'تفاصيل',
      syncNow: 'مزامنة الآن',
      submit: 'اضافة',
      syncing: 'جاري المزامنة...'
    },
    status: {
      online: 'متصل',
      offline: 'غير متصل',
      synced: 'متزامن',
      unsyncedCount: '{{count}} غير متزامنة'
    },
    placeholders: {
      search: 'ابحث...',
      optional: 'اختياري' ,
      addNotes: 'أضف ملاحظات...',
      searchCustomers: 'ابحث عن الزبائن...',
      searchProducts: 'ابحث عن المنتجات أو الموردين...',
      amountReceived: 'المبلغ المستلم...',
    },
   
    labels: {
      undo: 'تراجع',
      undoing: 'جاري التراجع...',
      actionCompleted: 'تم الإجراء',
      actionUndone: 'تم التراجع عن الإجراء',
      actionFailed: 'فشل الإجراء',
      actionFailedDescription: 'فشل الإجراء. يرجى المحاولة مرة أخرى.',
      actionFailedTitle: 'فشل الإجراء',
      undoActions: {
        delete_bill: 'تم حذف الفاتورة',
        update_bill: 'تم تعديل الفاتورة',
        complete_checkout: 'تم إتمام البيع',
        update_sale: 'تم تعديل البيع',
        delete_sale: 'تم حذف البيع',
        add_inventory_batch: 'تمت إضافة دفعة مخزون',
        update_inventory_batch: 'تم تعديل دفعة المخزون',
        delete_inventory_batch: 'تم حذف دفعة المخزون',
        apply_commission_rate: 'تم تطبيق نسبة العمولة',
        add_inventory_item: 'تمت إضافة صنف مخزون',
        update_inventory_item: 'تم تعديل صنف المخزون',
        delete_inventory_item: 'تم حذف صنف المخزون',
        archive_inventory_item: 'تمت أرشفة صنف المخزون',
        unarchive_inventory_item: 'تمت استعادة صنف المخزون',
        add_product: 'تمت إضافة منتج',
        update_product: 'تم تعديل المنتج',
        delete_product: 'تم حذف المنتج',
        add_customer: 'تمت إضافة زبون',
        update_customer: 'تم تعديل الزبون',
        add_supplier: 'تمت إضافة مورد',
        update_supplier: 'تم تعديل المورد',
        add_employee: 'تمت إضافة موظف',
        update_employee: 'تم تعديل الموظف',
        delete_employee: 'تم حذف الموظف',
        update_branch: 'تم تعديل الفرع',
        add_transaction: 'تمت إضافة معاملة',
        open_cash_drawer: 'تم فتح درج النقد',
        close_cash_drawer: 'تم إغلاق درج النقد',
        cash_drawer_transaction: 'معاملة درج النقد',
        supplier_advance_review: 'تم تسجيل سلفة المورد',
        supplier_advance_update: 'تم تعديل سلفة المورد',
        supplier_advance_delete: 'تم حذف سلفة المورد',
      },
    bill:'فاتورة', saleCompletedSuccessfully: 'تم إتمام البيع بنجاح', billCreatedAndReceiptPrinted: 'تم إنشاء الفاتورة وطباعة الإيصال',
      yourCartIsEmpty: 'سلة المشتريات فارغة', startAddingProductsToBeginYourSale: 'أضف المنتجات لبدء البيع',
      addNewCustomerSupplier: 'إضافة زبون جديد',
      walkInCustomer: 'زبون عابر',
      customerName: 'اسم الزبون',
      selectCustomer: 'اختر الزبون',
      paymentMethod: 'طريقة الدفع',
      all: 'الكل',
      cash: 'نقدي',
      card: 'بطاقة',
      change: 'الباقي',
      notes: 'الملاحظات',
      completeSale: 'إتمام البيع',
      addNewCustomer: 'إضافة زبون جديد',
      unitPrice: 'السعر',
      total: 'الإجمالي',
      weight: 'الوزن',
      quantity: 'الكمية',
      received: 'الاستلام',
      credit: 'دين',
      amountReceived: 'المبلغ المستلم',
      addNewProduct: 'إضافة منتج جديد', createNewProductForInventory: 'إنشاء منتج جديد لمخزونك', productInformation: 'معلومات المنتج', enterProductName: 'أدخل اسم المنتج', category: 'الفئة', fruits: 'الفواكه', vegetables: 'الخضروات', leafy: 'الحشائش', nuts: 'البقوليات', others: 'الأخرى', productImage: 'صورة المنتج', productPhoto: 'صورة المنتج', clickToUploadImage: 'انقر لتحميل الصورة', pngJpgUpTo5Mb: 'PNG, JPG حتى 5MB', cancel: 'إلغاء', adding: 'جاري الإضافة...', addProduct: 'إضافة المنتج',
      units: 'وحدات',
      kg: 'كغ',
      piece: 'قطعة',
      box: 'صندوق',
      bag: 'كيس',
      bundle: 'ربطة',
      dozen: 'دزينة',
      image: 'الصورة',
      name: 'الاسم',
      product: 'المنتج',
      supplier: 'المورد',
      type: 'النوع',
      available: 'متوفر',
      actions: 'الإجراءات'
    },
    confirmations: {
      fullResyncConfirm: 'سيتم مسح البيانات المحلية وإعادة تنزيلها من الخادم. هل تريد المتابعة؟'
    },
    alerts: {
      connectionRestored: 'تم استعادة الاتصال',
      autoSyncingChanges: 'جاري مزامنة تغييراتك تلقائيًا...'
    },
    currency: {
      USD: 'دولار أمريكي', LBP: 'ليرة لبنانية', EUR: 'يورو', GBP: 'جنيه إسترليني',
      SAR: 'ريال سعودي', AED: 'درهم إماراتي', EGP: 'جنيه مصري', JOD: 'دينار أردني',
      SYP: 'ليرة سورية', IQD: 'دينار عراقي', TRY: 'ليرة تركية', MAD: 'درهم مغربي',
      TND: 'دينار تونسي', DZD: 'دينار جزائري', LYD: 'دينار ليبي', SDG: 'جنيه سوداني',
      YER: 'ريال يمني', KWD: 'دينار كويتي', BHD: 'دينار بحريني', QAR: 'ريال قطري',
      OMR: 'ريال عماني',
    },
    previous: 'السابق',
    next: 'التالي',
    showing: 'عرض',
    to: 'إلى',
    of: 'من',
    results: 'نتائج',
    keyboard: {
      hide: 'إخفاء',
      clear: 'مسح',
      space: 'مسافة',
      numericKeyboard: 'لوحة المفاتيح الرقمية',
      textKeyboard: 'لوحة المفاتيح النصية',
      desktopTesting: '(اختبار سطح المكتب)',
      switchLanguage: 'تبديل اللغة',
      english: 'English',
      arabic: 'العربية'
    }
  },
  employees: {
    cashiers: 'المحاسبين',
    managers: 'المدراء',
    title: 'إدارة الموظفين',
    description: 'إدارة الموظفين، الأدوار، والساعات العملية',
    addEmployee: 'إضافة موظف',
    employeeSearch: 'ابحث عن الموظفين باسم، البريد الإلكتروني، الأدوار، أو الهاتف',
    editEmployee: 'تعديل الموظف',
    deleteEmployee: 'حذف الموظف',
    employeeInfo: 'معلومات الموظف',
    employeeModuleAccess: 'إدارة الصلاحيات النمطية',
    employeeOperationLimits: 'إدارة الحدود العملية',
    employeeDetails: 'تفاصيل الموظف',
    employeeRoles: 'أدوار الموظف',
    recordAPayment: 'تسجيل دفعة',
    employeeWorkingHours: 'ساعات العمل الموظف',
    workingDays: 'أيام العمل الموظف',
    workingHoursStart: 'وقت البدء للعمل',
    recordAPaymentTo: 'تسجيل دفعة ل',
    payEmployee: 'دفعة موظف',
    
    create: 'إنشاء',
    workingHoursEnd: 'وقت النهاية للعمل',
    accessDenied: 'تم رفض الوصول. يتطلب دور المدير',
    employeeDeletedSuccessfully: 'تم حذف الموظف بنجاح',
    employeeDeletedFailed: 'فشل حذف الموظف',
    employeeUpdatedSuccessfully: 'تم تعديل الموظف بنجاح',
    employeeUpdatedFailed: 'فشل تعديل الموظف',
    employeeAddedSuccessfully: 'تم إضافة الموظف بنجاح',
    employeeAddedFailed: 'فشل إضافة الموظف',
    employeeListTitle: 'قائمة الموظفين',
    employeeListDescription: 'إدارة الموظفين، الأدوار، والساعات العملية',
    employeeCreatedSuccessfullyWithLoginCredentials: 'تم إنشاء الموظف بنجاح ببيانات الدخول',
    employeeCreatedFailed: 'فشل إنشاء الموظف',
    employeeCreatedFailedWithLoginCredentials: 'فشل إنشاء الموظف ببيانات الدخول',
    employeeListFailed: 'فشل تحميل قائمة الموظفين',
    nameRequired: 'يتطلب الاسم',
    duplicateNameError: 'يوجد موظف بهذا الاسم بالفعل.',
    emailRequired: 'يتطلب البريد الإلكتروني',
    passwordRequired: 'يتطلب كلمة المرور',
    roleRequired: 'يتطلب الأدور',
    youCannotDeleteYourOwnAccount: 'لا يمكنك حذف حسابك الخاص',
    areYouSureYouWantToDeleteThisEmployee: 'هل أنت متأكد من أنك تريد حذف هذا الموظف؟',
    moduleAccessUpdatedSuccessfully: 'تم تعديل الصلاحيات النمطية بنجاح',
    operationLimitsUpdatedSuccessfully: 'تم تعديل الحدود العملية بنجاح',
    loadingEmployees: 'جاري تحميل الموظفين...',
    noEmployeesFound: 'لا يوجد موظفين',
    cashier: 'المحاسب',
    manager: 'المدير',
    clearSearch: 'مسح البحث',
    name: 'الاسم',
    role: 'الأدور',
    email: 'البريد الإلكتروني',
    phone: 'الهاتف',
    monthlySalary: 'الراتب الشهري',
    emailCannotBeChanged: 'لا يمكن تعديل البريد الإلكتروني',
    contact: 'الاتصال',
    salary: 'الراتب',
    workingHours: 'ساعات العمل',
    actions: 'الإجراءات',
    close: 'إغلاق',
    employeeWillUseThisPasswordToLogIn: 'سيستخدم الموظف هذه الكلمة السرية لتسجيل الدخول',
    selectWorkingDays: 'اختر الأيام المناسبة للعمل',
    daysSelected: 'يوم مختار',
    done: 'تم',
    monday: 'الاثنين',
    tuesday: 'الثلاثاء',
    wednesday: 'الأربعاء',
    thursday: 'الخميس',
    friday: 'الجمعة',
    saturday: 'السبت',
    sunday: 'الأحد',
    clickToSelectWorkingDays: 'انقر لاختيار الأيام المناسبة للعمل',
    update: 'تعديل',
    cancel: 'إلغاء',
    address: 'العنوان',
    notSet: 'غير محدد',
    password: 'كلمة المرور',
    minimum6Characters: 'يجب أن يكون لديك 6 أحرف على الأقل',
  },
  nav: {
    employees: 'الموظفين',
    home: 'الرئيسية',
    inventory: 'المخزون',
    lowStock: 'مخزون منخفض',
    pos: 'نقطة البيع',
    accounts: 'الحسابات',
    accounting: 'المحاسبة',
    reports: 'التقارير',
    unsynced: 'العناصر غير المتزامنة',
    settings: 'الإعدادات',
    auditLog: 'سجل التدقيق'
  },
  layout: {
    title: 'ProducePOS',
    connection: {
      online: 'متصل',
      offline: 'غير متصل'

    },
    unsynced: 'غير متزامنة',
    signOut: 'تسجيل الخروج'
  },
  sync: {
    backgroundHydration: 'إكمال المزامنة في الخلفية…',
    outboxPermanentSection: 'رفع فاشل (لا يُعاد تلقائياً)',
    discardOutbox: 'إزالة من الطابور',
    permissionRevokedTitle: 'انتهت الجلسة',
    permissionRevokedDetail: 'لم يعد لديك صلاحية لهذا المتجر. سجّل الدخول مرة أخرى إذا تغيّت صلاحياتك.',
  },
  login: {
    title: 'نظام  البيع ',
    subtitle: 'نظام  موارد المؤسسات لسوق الخضار بالجملة',
    email: 'البريد الإلكتروني',
    emailPlaceholder: 'أدخل بريدك الإلكتروني',
    password: 'كلمة المرور',
    passwordPlaceholder: 'أدخل كلمة المرور',
    signIn: 'تسجيل الدخول',
    signUp: 'إنشاء حساب',
    signingIn: 'جاري تسجيل الدخول...',
    signingUp: 'جاري إنشاء الحساب...',
    fullName: 'الاسم الكامل',
    role: 'الدور',
    store: 'المتجر',
    selectStore: 'اختر المتجر',
    demoAccount: 'حساب تجريبي',
    invalidCredentials: 'البريد الإلكتروني أو كلمة المرور غير صحيحة',
    signupFailed: 'فشل إنشاء الحساب. حاول مجددًا.',
    genericError: 'حدث خطأ. حاول مجددًا.',
    lockedTitle: 'محاولات فاشلة كثيرة',
    lockedMessage: 'تم تعطيل تسجيل الدخول مؤقتًا. حاول مجددًا خلال {{time}}.'
  },
  settings: {
    header: 'الإعدادات',
    saved: 'تم حفظ الإعدادات بنجاح!',
    userInfo: 'معلومات المستخدم',
    name: 'الاسم',
    email: 'البريد الإلكتروني',
    role: 'الدور',
    inventoryAlerts: 'تنبيهات المخزون',
    lowStockAlerts: 'تنبيهات انخفاض المخزون',
    lowStockDescription: 'استلم تنبيهًا عند انخفاض كمية المنتج',
    lowStockThreshold: 'حد المخزون المنخفض',
    units: 'وحدات',
    save: 'حفظ',
    currentThreshold: 'الحد الحالي: {{value}} وحدات',
    commissionSettings: 'إعدادات العمولة',
    defaultCommissionRate: 'نسبة العمولة الافتراضية',
    currentDefaultRate: 'النسبة الافتراضية الحالية: {{value}}%',
    currencySettings: 'إعدادات العملة',
    displayCurrency: 'عملة العرض',
    currentCurrency: 'العملة الحالية: {{value}}',
    systemInfo: 'معلومات النظام',
    appVersion: 'إصدار التطبيق',
    dataStorage: 'تخزين البيانات',
    lastSync: 'آخر مزامنة',
    deviceType: 'نوع الجهاز',
    webApp: 'تطبيق ويب',
    security: 'الأمان',
    sessionManagement: 'إدارة الجلسة',
    sessionNote: 'ستظل جلستك نشطة حتى تقوم بتسجيل الخروج يدويًا',
    changePassword: 'تغيير كلمة المرور',
    language: 'اللغة',
    language_ar: 'العربية',
    language_en: 'الإنجليزية',
    language_fr: 'الفرنسية',
    branchInfo: 'معلومات الفرع'
  },
  home: {
    products: 'المنتجات',
    sales: 'المبيعات',
    opened: 'تم فتح الدرج',
    today: 'اليوم',
    week: 'الأسبوع',
    shortcuts: 'الاختصارات',
    welcome: 'أهلًا بعودتك، {{name}}',
    subtitle: 'هذا ما يحدث في متجرك اليوم.',
    fastActions: 'إجراءات سريعة',
    hide: 'إخفاء',
    show: 'إظهار',
    quickSale: 'بيع سريع',
    quickSaleDesc: 'ابدأ عملية بيع جديدة',
    receiveProducts: 'استلام منتجات',
    receiveProductsDesc: 'إضافة مخزون جديد من الموردين',
    addCustomer: 'إضافة زبون',
    addCustomerDesc: 'تسجيل زبون جديد',
    recordExpense: 'تسجيل مصروف',
    recordExpenseDesc: 'تسجيل مصروفات العمل',
    todaySales: 'مبيعات اليوم',
    todaySalesDesc: 'عرض أداء المبيعات',
    checkStock: 'فحص المخزون',
    checkStockDesc: 'مراقبة مستويات المخزون',
    cashInDrawer: 'النقد في الدرج ({{currency}})',
    notOpenedToday: 'لم يفتح اليوم',
    openCashDrawer: 'فتح درج النقد',  
    todaysExpenses: 'مصروفات اليوم ({{currency}})',
    todaysIncome: 'إيرادات اليوم ({{currency}})',
    sessionExpenses: 'مصروفات الجلسة ({{currency}})',
    sessionIncome: 'إيرادات الجلسة ({{currency}})',
    session: 'هذه الجلسة',
    lowStockItems: 'أصناف منخفضة المخزون',
    showBothCurrencies: 'إظهار العملتين',
    showCombined: 'إظهار الرصيد المجمع',
    needAttention: 'يتطلب الانتباه',
    alertsDisabled: 'التنبيهات معطلة',
    lowStockAlert: 'تنبيه انخفاض المخزون',
    allWellStocked: 'جميع المنتجات مخزونها جيد!',
    recentActions: 'الإجراءات الأخيرة',
    noRecentSales: 'لا توجد إجراءات حديثة'
  },
  inventory: {
    currencyNotAccepted: 'هذه العملة غير مقبولة في هذا المتجر. المقبولة: {{list}}',
    currencyRequired: 'اختر العملة قبل الحفظ.',
    missingCurrency: 'هذا الصف لا يحتوي على عملة (بيانات قديمة).',
    batchFeeCurrency: 'عملة رسوم الدفعة',
    lineItemCurrency: 'عملة السطر',
    currency: 'العملة',
    fixInInventoryHint: 'صحح من المخزون',
    insufficientCashDrawerBalance: 'رصيد الدرج غير كافي. الرصيد الحالي: {{currentBalance}}, المطلوب: {{requiredAmount}}',
    failedToReceiveInventory: 'فشل في استلام المخزون.',
    supplierRequiredForCommissionPurchases: 'يتطلب المورد لشراء بالعمولة.',
    pleaseSelectADifferentSupplier: 'يرجى اختيار مورد آخر.',
    quantityMustBeAtLeast1: 'يجب أن يكون لديك 1 أو أكثر.',
    priceRequiredForCashPurchases: 'يتطلب السعر ويجب أن يكون أكثر من 0 للشراء بالنقد.',
    bothWeightOrQuantityIsRequiredForCashPurchases: 'يتطلب الوزن أو الكمية للشراء بالنقد.',
    supplierRequiredForCreditPurchases: 'يتطلب المورد لشراء بالائتمان.',
    plasticPriceRequiredWhenPlasticMortgageIsChecked: 'يتطلب السعر البلاستيكي عند الرهن البلاستيكي.',
    plasticCountRequiredWhenPlasticMortgageIsChecked: 'يتطلب عدد البلاستيكات عند الرهن البلاستيكي.',
    receivedDateRequired: 'يتطلب تاريخ الاستلام.',
    receivedDateCannotBeInTheFuture: 'تاريخ الاستلام لا يمكن أن يكون في المستقبل.',
    default: 'افتراضي',
    skuBarcode: 'رمز/Barcode',
    supplierName: 'اسم المورد',
    phone: 'الهاتف',
    email: 'البريد الإلكتروني',
    type: 'النوع',
    typeCommission: 'عمولة',
    typeCash: 'نقدي',
    address: 'العنوان',
    cancel: 'إلغاء',
    adding: 'جاري الإضافة...',
    addSupplier: 'إضافة مورد',
    editBatch: 'تعديل الدفعة',
    updateBatchInformationAndInventoryItems: 'تحديث معلومات الدفعة والمنتجات',
    addNewInventoryItemsToYourStock: 'إضافة منتجات جديدة للمخزون',
    cashPurchase: 'شراء نقدي',
    creditPurchase: 'شراء بالدين',
    commission: 'عمولة',
    supplierInformation: 'معلومات المورد',
    supplyType: 'نوع التوريد',
    commissionRate: 'نسبة العمولة',
    financialInformation: 'معلومات مالية',
    additionalInformation: 'معلومات إضافية',
    receivedDate: 'تاريخ الاستلام',
    referenceNumber: 'رقم المرجع',
    plasticMortgage: 'رهن صناديق',
    numberOfPlastics: 'عدد البلاستيكات',
    plasticNumber: 'رقم البلاستيك',
    plasticPrice: 'سعر البلاستيك',
    product: 'المنتج',
    weight: 'الوزن',
    qty: 'الكمية',
    unit: 'الوحدة',
    price: 'السعر',
    sellingPrice: 'سعر البيع',
    removeProduct: 'إزالة المنتج',
    cannotRemoveExistingItem: 'لا يمكن إزالة المنتج الموجود',
    updateBatch: 'تحديث الدفعة',
    kg: 'كغ',
    products: 'المنتجات',
    editExistingItemsOrAddNewOnes: 'تعديل المنتجات الموجودة أو إضافة منتجات جديدة',
    trade: 'تجارة',
    selectProduct: 'اختر المنتج',
    notes: 'ملاحظات',
    addAnyAdditionalStatusOrComments: 'إضافة ملاحظات إضافية',
    porterageFee: 'رسوم عتالة (اختياري)',
    transferFee: 'رسوم نقل (اختياري)',
    costSummary: 'ملخص التكلفة',
    additionalFees: 'رسوم إضافية',
    totalCost: 'إجمالي التكلفة',
    notesOptional: 'ملاحظات (اختياري)',
    addNewProductTitle: 'إضافة منتج جديد',
    addNewProductDesc: 'إنشاء منتج جديد للمخزون',
    productName: 'اسم المنتج',
    category: 'الفئة',
    categoryFruits: 'فواكه',
    categoryTropicalFruits: 'فواكه استوائية',
    categoryVegetables: 'خضروات',
    categoryHerbs: 'حشائش',
    categoryGrains: 'حبوب',
    categoryNuts: 'مكسرات',
    categoryOthers: 'أخرى',
    productPhotoOptional: 'صورة المنتج (اختياري)',
    uploadImageCta: 'انقر لرفع الصورة',
    uploadImageHint: 'PNG, JPG حتى 5MB',
    editProductTitle: 'تعديل المنتج',
    header: 'إدارة المخزون',
    receiveProducts: 'استلام منتجات',
    addProduct: 'إضافة منتج',
    productReception: 'استلام المنتجات',
    stockProducts: 'منتجات المخزون',
    searchProducts: 'ابحث عن المنتجات...',
    currentStockLevels: 'مستويات المخزون الحالية',
    outOfStock: 'نفد المخزون',
    lowStock: 'مخزون منخفض',
    inStock: 'متوفر',
    recentProductReceives: 'أحدث الاستلامات',
    actions: 'إجراءات',
    edit: 'تعديل',
    delete: 'حذف',
    archive: 'أرشفة',
    unarchive: 'إلغاء الأرشفة',
    archivedItems: 'العناصر المؤرشفة',
    archiveSuccess: 'تمت أرشفة العنصر بنجاح',
    unarchiveSuccess: 'تمت استعادة العنصر بنجاح',
    archiveFailure: 'فشل في أرشفة العنصر',
    unarchiveFailure: 'فشل في استعادة العنصر',
    noArchivedItems: 'لا توجد عناصر مؤرشفة',
    noArchivedItemsDesc: 'العناصر التي تؤرشفها من تبويب المخزون ستظهر هنا',
    hasReferencesTitle: 'لا يمكن الحذف — توجد مبيعات',
    hasReferencesDesc: 'هذا العنصر مرتبط بـ {{count}} عملية بيع. تم منع الحذف لحماية سجلاتك المالية. يمكنك أرشفته بدلاً من ذلك لوقف بيعه.',
    archiveConfirmTitle: 'أرشفة عنصر المخزون',
    archiveConfirmDesc: 'سيتم إخفاء هذا العنصر من المبيعات. يمكنك استعادته في أي وقت من تبويب المؤرشف.',
    deleteConfirmTitle: 'حذف عنصر المخزون',
    deleteConfirmDesc: 'ليس لهذا العنصر أي مبيعات. سيتم حذفه نهائياً.',
    permanentDeleteTitle: 'حذف نهائي',
    permanentDeleteDesc: 'سيتم إزالة العنصر بشكل دائم. لا يمكن التراجع عن هذا الإجراء.',
    selectAll: 'تحديد الكل',
    deselectAll: 'إلغاء التحديد',
    selectedCount: '{{count}} محدد',
    bulkUnarchive: 'استعادة المحدد',
    bulkDelete: 'حذف المحدد',
    confirmBulkDelete: 'حذف {{count}} عنصر(عناصر) نهائياً؟ لا يمكن التراجع.',
    noContactInfo: 'لا توجد معلومات اتصال',
    remaining: 'متبقي',
    unitLabel: 'الوحدة',
    quantityLabel: 'الكمية',
    weightOptional: 'الوزن (اختياري)',
    priceOfPlastics: 'سعر البلاستيكات',
    noProducts: 'لا توجد منتجات',
    showAsTagInPOSPage: 'عرض كعلامة في صفحة نقطة البيع'

  },
  pos: {
    pleaseWaitWhileYourReceiptIsBeingPrinted: 'يرجى الإنتظار أثناء طباعة الإيصال',
    ProcessingSale: 'جاري المعالجة...',
    PrintingReceipt: 'جاري الطباعة...',
    Loading: 'جاري التحميل...',
    header: 'نقطة البيع',
    newBill: 'فاتورة جديدة',
    cartEmpty: 'السلة فارغة',
    products: 'المنتجات',
    subtotal: 'الإجمالي الفرعي',
    total: 'الإجمالي',
    customerName: 'اسم الزبون',
    walkInCustomer: 'زبون عابر',
    paymentMethod: 'طريقة الدفع',
    cash: 'نقدًا',
    card: 'بطاقة',
    credit: 'دين',
    amountReceived: 'المبلغ المستلم',
    change: 'الباقي',
    notesOptional: 'ملاحظات (اختياري)',
    completeSale: 'إتمام البيع',
    processing: 'جارٍ المعالجة...',
    addNewCustomer: 'إضافة زبون جديد',
    addNewCustomerTitle: 'إضافة زبون جديد',
    nameLabel: 'الاسم',
    phoneLabel: 'الهاتف',
    emailLabel: 'البريد الإلكتروني',
    addressLabel: 'العنوان',
    isActive: 'نشط',
    qty: 'الكمية',
    weight: 'الوزن',
    price: 'السعر',
    searchCustomers: 'ابحث عن الزبائن...',
    searchProducts: 'ابحث عن المنتجات أو الموردين...'
  },
  reports: {
    header: 'التقارير والتحليلات',
    exportReport: 'تصدير التقرير',
    generateReport: 'إنشاء تقرير',
    reportType: 'نوع التقرير',
    startDate: 'تاريخ البدء',
    endDate: 'تاريخ الانتهاء',
    salesReport: 'تقرير المبيعات',
    inventoryReport: 'تقرير المخزون',
    customerReport: 'تقرير الزبائن',
    profitAnalysis: 'تحليل الربح',
    totalRevenue: 'إجمالي الإيرادات',
    totalSales: 'إجمالي المبيعات',
    averageSale: 'متوسط البيع',
    customerDebt: 'ديون الزبائن',
    topSellingProducts: 'أفضل المنتجات مبيعًا',
    currentStockLevels: 'مستويات المخزون الحالية',
    status: {
      outOfStock: 'نفد المخزون',
      lowStock: 'مخزون منخفض',
      inStock: 'متوفر',
      never: 'أبدًا'
    },
    trialBalance: {
      title: 'ميزان المراجعة',
      description: 'جميع حسابات دفتر الأستاذ مع إجماليات المدين والدائن للفترة المحددة. يجب أن يتطابق المدين مع الدائن.',
      tableTitle: 'أرصدة الحسابات',
      filters: 'الفلاتر',
      quickDateRange: 'فترة سريعة',
      startDate: 'تاريخ البدء',
      endDate: 'تاريخ الانتهاء',
      preset: { today: 'اليوم', week: 'أسبوع', mtd: 'الشهر حتى اليوم', month: '٣٠ يومًا', year: 'السنة حتى اليوم' },
      postedOnly: 'القيود المرحَّلة فقط',
      compareWithPrevious: 'مقارنة بالفترة السابقة',
      clearFilters: 'مسح الفلاتر',
      exportCsv: 'تصدير CSV',
      print: 'طباعة',
      close: 'إغلاق',
      retry: 'إعادة المحاولة',
      loading: 'جارٍ تحميل ميزان المراجعة...',
      errorTitle: 'تعذّر تحميل ميزان المراجعة',
      noEntries: 'لا توجد قيود في هذه الفترة.',
      totals: 'الإجمالي',
      totalDebits: 'إجمالي المدين',
      totalCredits: 'إجمالي الدائن',
      difference: 'الفرق',
      balanced: 'متوازن',
      unbalanced: 'غير متوازن',
      accountCount: '{{count}} حسابًا بنشاط',
      drillDownTitle: 'قيود حساب يومية',
      drillDownEmpty: 'لا توجد قيود لهذا الحساب في الفترة المحددة.',
      drillDownEntryCount: '{{count}} قيدًا',
      col: {
        accountCode: 'رمز الحساب',
        accountName: 'اسم الحساب',
        accountType: 'النوع',
        debit: 'مدين',
        credit: 'دائن',
        balance: 'الرصيد',
        postedDate: 'تاريخ الترحيل',
        description: 'الوصف',
        transactionId: 'العملية',
        billId: 'الفاتورة'
      },
      accountTypes: {
        asset: 'أصول',
        liability: 'خصوم',
        equity: 'حقوق الملكية',
        revenue: 'إيرادات',
        expense: 'مصاريف'
      }
    },
    balanceSheet: {
      title: 'الميزانية العمومية',
      loading: 'جاري تحميل الميزانية العمومية...',
      retry: 'إعادة المحاولة',
      showZeroBalances: 'إظهار الأرصدة الصفرية',
      allAccounts: 'كل الحسابات',
      account: 'الحساب',
      amount: 'المبلغ',
      equation: 'الأصول = الخصوم + حقوق الملكية',
      variance: 'الفرق',
      sections: {
        current_asset: 'الأصول المتداولة',
        non_current_asset: 'الأصول غير المتداولة',
        current_liability: 'الخصوم المتداولة',
        non_current_liability: 'الخصوم غير المتداولة',
        equity: 'حقوق الملكية'
      },
      accounts: {
        '1100': 'النقدية',
        '1200': 'الذمم المدينة',
        '1300': 'المخزون',
        '1400': 'المصاريف المدفوعة مقدماً',
        '1500': 'المعدات',
        '2100': 'الذمم الدائنة',
        '2200': 'الرواتب المستحقة',
        '2300': 'قروض قصيرة الأجل',
        '3100': 'حقوق الملكية',
        '3200': 'الأرباح المحتجزة',
        '4100': 'إيرادات المبيعات',
        '4200': 'إيرادات الخدمات',
        '4300': 'إيرادات أخرى',
        '5100': 'تكلفة البضاعة المباعة',
        '5200': 'مصاريف الرواتب',
        '5300': 'مصاريف الإيجار',
        '5400': 'مصاريف المرافق',
        '5500': 'اللوازم المكتبية',
        '5600': 'مصاريف التسويق',
        '5700': 'الأتعاب المهنية',
        '5800': 'الرسوم البنكية',
        '5900': 'مصاريف متنوعة',
        '5950': 'خسائر المخزون / نقص الوزن',
        CYE: 'أرباح السنة الحالية'
      }
    }
  },
  syncStatus: {
    header: 'حالة المزامنة',
    connection: 'الاتصال',
    lastSync: 'آخر مزامنة',
    pendingChanges: 'تغييرات قيد الانتظار',
    items: 'عناصر',
    manualSync: 'مزامنة يدوية',
    fullResync: 'إعادة مزامنة كاملة',
    validateAndClean: 'تحقق ونظف البيانات',
    workingOffline: 'تعمل دون اتصال',
    offlineNote: 'سيتم مزامنة التغييرات تلقائيًا عند استعادة الاتصال.',
    autoSyncEnabled: 'المزامنة التلقائية مفعلة • تتم المزامنة تلقائيًا عند الاتصال'
  },
  qrCode: {
    generating: 'جاري إنشاء رمز QR...',
    error: 'خطأ',
    scanForAccountStatement: 'امسح للحصول على كشف الحساب',
    bill: 'الفاتورة',
    failedToLoad: 'فشل في تحميل صورة رمز QR',
    missingInfo: 'معلومات الزبون أو الفاتورة مفقودة',
    generateFailed: 'فشل في إنشاء رمز QR'
  },
  publicStatement: {
    loading: 'جاري تحميل كشف الحساب...',
    unableToLoad: 'تعذر تحميل الكشف',
    close: 'إغلاق',
    accountStatement: 'كشف الحساب',
    scannedFromQR: 'تم مسحه من رمز QR على الإيصال',
    mobileView: 'عرض الهاتف',
    qrCodeAccess: 'الوصول عبر رمز QR',
    qrIntegrationDemo: 'عرض توضيحي لتكامل رمز QR',
    qrDemoDescription: 'توضح هذه الصفحة كيف يمكن للعملاء الوصول إلى كشوف حساباتهم عن طريق مسح رموز QR من إيصالاتهم.',
    qrUrlStructure: 'هيكل رابط رمز QR:',
    customerInfo: 'معلومات الزبون',
    customerId: 'رقم الزبون',
    billId: 'رقم الفاتورة',
    implementationRequirements: 'متطلبات التنفيذ',
    createPublicAPI: 'إنشاء نقاط نهاية API عامة لتقديم بيانات الزبائن والفواتير',
    implementAuthFree: 'تنفيذ الوصول بدون مصادقة لكشوف الحسابات',
    addQRGeneration: 'إضافة إنشاء رمز QR لعملية إنشاء الفاتورة',
    integrateQRPrinting: 'دمج رموز QR في طباعة الإيصالات',
    ensureMobileResponsive: 'ضمان التصميم المتجاوب للهاتف المحمول للمسح',
    printStatement: 'طباعة الكشف',
    downloadPDF: 'تحميل PDF',
    qrCodeAccessFooter: 'تم الوصول إلى كشف الحساب هذا عبر رمز QR من إيصالك',
    generatedOn: 'تم إنشاؤه في'
  },
  customers: {
    customer: 'الزبون',
    paymentDirection: {
      paymentDirection: 'اتجاه الدفع',
      theyPayUs: 'يدفعون لنا',
      wePayThem: 'ندفع لهم',
    },
    initialBalance: 'الرصيد الافتتاحي',
    /** Parametric — pass `{ currency }`. Replaces initial{USD,LBP}Balance. */
    initialBalanceFor: 'الرصيد الافتتاحي بـ{{currency}}',
    /** @deprecated Use `initialBalanceFor`. */
    initialUSDBalance: 'الرصيد الافتتاحي بالدولار',
    /** @deprecated Use `initialBalanceFor`. */
    initialLBPBalance: 'الرصيد الافتتاحي بالليرة اللبنانية',
    paymentDescriptionPlaceholder: 'مثال: دفعة للمنتجات, دفعة للعمولة, etc.',
    tryAdjustingTheDateRangeOrCheckingBackLater: 'حاول ضبط الفترة الزمنية أو التحقق مرة أخرى لاحقا',
    addPaymentReceived: 'إضافة دفعة مستلمة',
    recordPaymentReceivedFromCustomer: 'تسجيل دفعة مستلمة من الزبون',
    recordPayment: 'تسجيل دفعة',
    employees: 'الموظفين',
    selectSupplier: 'اختر المورد *',
    searchSuppliers: 'ابحث عن الموردين...',
    addNewSupplier: 'إضافة مورد جديد',
    hours: 'الساعات',
    notes: 'الملاحظات',
    checkedIn: 'تم الحضور',
    checkedOut: 'تم الانصراف',
    employeePayments: 'دفعات الموظفين',
    employeePaymentsAndAttendance: 'دفعات وحضور الموظفين',
    checkIn: 'الحضور',
    checkOut: 'الانصراف',
    employee: 'الموظف',
    balance: 'الرصيد',
    status: 'الحالة',
    todayHours: 'الساعات اليومية',
    actions: 'الإجراءات',
    summaryAccountStatement: ' كشف الحساب',
    detailedAccountStatement: 'كشف حساب مفصل',
    period: 'فترة',
    priceAdjustment: 'تعديل السعر',
    from: 'من',
    to: 'إلى',
    financialOverview: 'نظرة عامة على المالي',
    openingBalance: 'الرصيد الافتتاحي',
    description: 'الوصف',
    descriptionPlaceholder: 'الوصف (اختياري)',
    processPayment: 'إجراء الدفع',
    attendanceHistory: 'سجل الحضور',
    closingBalance: 'الرصيد الافتتاحي',
    totalDebt: 'إجمالي الدين',
    totalPayments: 'إجمالي الدفعات',
    totalSales: 'إجمالي المبيعات',
    totalExpenses: 'إجمالي المصروفات',
    totalProfit: 'إجمالي الربح',
    noActiveAdvances: 'لا توجد سلفات مدفوعة للموردين',
    balanceSettings: 'إعدادات الرصيد',
    /** Parametric — pass `{ currency }`. Replaces max{LBP,USD}Balance. */
    maxBalanceFor: 'الحد الأقصى للرصيد بـ{{currency}} (اختياري)',
    /** @deprecated Use `maxBalanceFor`. */
    maxLBPBalance: 'الحد الأقصى لرصيد بالليرة اللبنانية',
    /** @deprecated Use `maxBalanceFor`. */
    maxUSDBalance: 'الحد الأقصى للرصيد بالدولار',
    noLimit: 'لا حدود',
    reviewDate: 'تاريخ المراجعة',
    reviewDateHint: 'ضبط تاريخ لمراجعة السلفة',
    allTime: 'كل الاوقات',
    today: 'اليوم',
    thisWeek: 'هذا الأسبوع',
    thisMonth: 'هذا الشهر',
    given: 'مدفوع',
    deducted: 'مخصوم',
    allCurrencies: 'جميع العملات',
    noAdvancesRecorded: 'لم يتم العثور على سلفات مدفوعة للموردين',
    advanceHistory: 'سجل السلفات',
    recordAdvancePayment: 'تسجيل سلفة',
    supplierAdvances: 'الموردين المسلفين',
    supplierAdvancesDesc: 'تتبع وإدارة السلفة للموردين',
    supplierAdvancesTitle: 'الموردين المسلفين',
    initialAdvancePaymentOptional: 'سلفة اولية (اختياري)',
    youCanAlsoManageAdvancesLaterFromTheSupplierAdvancesTabInAccounting: 'يمكنك أيضا التعامل مع السلفات لاحقا من علامة التبويب المسلفات في المحاسبة.',
    /** Parametric — pass `{ currency }`. Replaces advance{USD,LBP}. */
    advanceFor: 'سلفة بـ{{currency}}',
    /** @deprecated Use `advanceFor`. */
    advanceUSD: 'سلفة بالدولار',
    /** @deprecated Use `advanceFor`. */
    advanceLBP: 'سلفة بالليرة اللبنانية',
    giveAdvance: 'إعطاء سلفة',
    deductAdvance: 'خصم سلفة',
    /** Parametric — pass `{ currency }`. Replaces totalAdvances{USD,LBP}. */
    totalAdvancesFor: 'إجمالي السلف بـ{{currency}}',
    /** @deprecated Use `totalAdvancesFor`. */
    totalAdvancesUSD: 'إجمالي السلف بالدولار',
    /** @deprecated Use `totalAdvancesFor`. */
    totalAdvancesLBP: 'إجمالي السلف بالليرة اللبنانية',
    supplierRecordAdvance: 'تسجيل سلفة',
    suppliersWithAdvances: 'الموردين المسلفين',
    totalTransactions: 'إجمالي المعاملات',
    supplierTotalAdvancesDesc: 'إجمالي السلف للموردين',
    supplierTotalAdvancesTitle: 'إجمالي السلف',
    title: 'إدارة الحسابات',
    addCustomer: 'إضافة زبون',
    addSupplier: 'إضافة مورد',
    customers: 'الزبائن',
    clients: 'الزبائن',
    suppliers: 'الموردين',
    searchPlaceholder: 'ابحث عن {{type}} بالاسم أو الهاتف أو البريد الإلكتروني...',
    allCustomers: 'جميع الزبائن',
    allSuppliers: 'جميع الموردين',
    noCustomersFound: 'لم يتم العثور على زبائن.',
    noSuppliersFound: 'لم يتم العثور على موردين.',
    name: 'الاسم',
    contact: 'الاتصال',
    active: 'نشط',
    inactive: 'غير نشط',
    editCustomer: 'تعديل الزبون',
    editSupplier: 'تعديل المورد',
    makePayment: 'إجراء دفعة',
    viewAccountStatement: 'عرض كشف الحساب',
    addNewCustomer: 'إضافة زبون جديد',
    editCustomerTitle: 'تعديل الزبون',
    editSupplierTitle: 'تعديل المورد',
    phone: 'الهاتف',
    email: 'البريد الإلكتروني',
    address: 'العنوان',
    nameRequired: 'الاسم والهاتف مطلوبان.',
    nameLabel: 'الاسم *',
    phoneLabel: 'الهاتف *',
    emailLabel: 'البريد الإلكتروني',
    addressLabel: 'العنوان',
    paymentAmount: 'مبلغ الدفعة',
    paymentCurrency: 'عملة الدفعة',
    paymentDescription: 'وصف الدفعة',
    paymentReference: 'مرجع الدفعة',
    usd: '$',
    lbp: 'ل.ل',
    owes: 'مدين',
    credit: 'رصيد دائن',
    paid: 'مسدد',
    quickPay: 'اقتراحات الدفع السريع',
    overpaymentWarning: 'تنبيه دفع زائد',
    overpaymentMessage: 'هذا الدفع يتجاوز الدين الحالي. سيكون للجهة رصيد دائن قدره',
    save: 'حفظ',
    cancel: 'إلغاء',
    close: 'إغلاق',
    duplicateNameError: 'يوجد زبون بهذا الاسم بالفعل.',
    duplicateSupplierNameError: 'يوجد مورد بهذا الاسم بالفعل.'
  },
  accounting: {
    tabs: {
      dashboard: 'لوحة التحكم',
      nonpriced: 'عناصر غير مسعّرة',
      billsManagement: 'إدارة الفواتير',
      receivedBills: 'الفواتير المستلمة',
      payments: 'المدفوعات الأخيرة',
      cashDrawer: 'درج النقود',
    },
    errorLoadingData: 'خطأ في تحميل البيانات',
    unableToLoadData: 'غير قادر على تحميل بيانات المحاسبة. يرجى تحديث الصفحة أو التحقق من الاتصال.',
    loadingData: 'جاري تحميل بيانات المحاسبة...',
    cashDrawerManagement: 'إدارة درج النقود',
    deleteCustomer: 'حذف الزبون',
    pleaseEnterValidAmount: 'يرجى إدخال مبلغ صحيح',
    pleaseSelectEntity: 'يرجى اختيار عميل',
    entityNotFound: 'العميل غير موجود',
    paymentReceived: 'تم استلام الدفعة! {{amount}} من {{entityName}}',
    paymentSent: 'تم إرسال الدفعة! {{amount}} إلى {{entityName}}',
    failedToRecordPayment: 'فشل في تسجيل الدفعة.',
    expenseRecorded: 'تم تسجيل المصروف! {{amount}} لـ {{categoryName}}',
    failedToRecordExpense: 'فشل في تسجيل المصروف.',
    pleaseEnterValidUnitPrice: 'يرجى إدخال سعر وحدة صحيح',
    pleaseEnterValidQuantity: 'يرجى إدخال كمية صحيحة',
    itemUpdatedSuccessfully: 'تم تحديث العنصر بنجاح',
    errorUpdatingItem: 'خطأ في تحديث العنصر',
    setValidPriceBeforeMarking: 'حدد سعراً صحيحاً قبل وضع علامة التسعير.',
    setValidQuantityBeforeMarking: 'حدد كمية صحيحة قبل وضع علامة التسعير.',
    itemMarkedAsPriced: 'تم وضع علامة التسعير على العنصر بنجاح!',
    errorMarkingAsPriced: 'خطأ في وضع علامة التسعير على العنصر',
    noValidItemsSelected: 'لم يتم اختيار عناصر صحيحة (يجب أن تحتوي العناصر على سعر وكمية)',
    itemsMarkedAsPriced: 'تم وضع علامة التسعير على {{count}} عنصر بنجاح!',
    errorMarkingItemsAsPriced: 'خطأ في وضع علامة التسعير على العناصر',
    itemDeletedSuccessfully: 'تم حذف العنصر بنجاح',
    errorDeletingItem: 'خطأ في حذف العنصر',
    itemsDeletedSuccessfully: 'تم حذف العناصر بنجاح',
    errorDeletingItems: 'خطأ في حذف العناصر',
    inventoryLogsExported: 'تم تصدير سجلات معاملات المخزون بنجاح',
    errorProcessingPendingBills: 'خطأ في معالجة بيانات الفواتير المعلقة',
    billAlreadyClosed: 'هذه الفاتورة مغلقة بالفعل.',
    billClosedSuccessfully: 'تم إغلاق الفاتورة بنجاح! تم خصم جميع الرسوم وتحديث رصيد المورد.',
    batchUpdatedSuccessfully: 'تم تحديث الدفعة بنجاح',
    errorUpdatingBatch: 'خطأ في تحديث الدفعة',
    commissionRateApplied: 'تم تطبيق معدل العمولة بنجاح',
    errorApplyingCommissionRate: 'خطأ في تطبيق معدل العمولة',
    billClosedCommissionDeducted: 'تم إغلاق الفاتورة بنجاح! تم خصم العمولة وتسجيل دفعة المورد.',
    failedToCloseBill: 'فشل في إغلاق الفاتورة. يرجى المحاولة مرة أخرى.',
    noCompletedBillsSelected: 'لم يتم اختيار فواتير مكتملة للإغلاق',
    successfullyClosedBills: 'تم إغلاق {{count}} فاتورة بنجاح',
    errorClosingBills: 'خطأ في إغلاق الفواتير',
    pendingBillsExported: 'تم تصدير الفواتير المعلقة بنجاح',
    errorExportingPendingBills: 'خطأ في تصدير الفواتير المعلقة',
    errorProcessingReceivedBills: 'خطأ في معالجة بيانات الفواتير المستلمة',
    receivedBillsExported: 'تم تصدير الفواتير المستلمة بنجاح',
    errorExportingReceivedBills: 'خطأ في تصدير الفواتير المستلمة',
    saleUpdatedSuccessfully: 'تم تحديث البيع بنجاح',
    errorUpdatingSale: 'خطأ في تحديث البيع',
    saleDeletedSuccessfully: 'تم حذف البيع بنجاح',
    errorDeletingSale: 'خطأ في حذف البيع'
  },
  dashboard: {
    createdBy: 'تم إنشاء بواسطة',
    allStatuses: 'جميع الحالات',
    filters: 'المرشحات',
    active: 'نشط',
    less: 'أقل',
    more: 'المزيد',
    reset: 'إعادة تعيين',
    searchTransactions: 'البحث في المعاملات...',
    allTypes: 'جميع الأنواع',
    income: 'الدخل',
    expense: 'المصروف',
    allCurrencies: 'جميع العملات',
    startDate: 'تاريخ البداية',
    endDate: 'تاريخ النهاية',
    minAmount: 'الحد الأدنى للمبلغ',
    maxAmount: 'الحد الأقصى للمبلغ',
    sortBy: 'ترتيب حسب:',
    date: 'التاريخ',
    amount: 'المبلغ',
    category: 'الفئة',
    cashDrawerBalance: 'رصيد درج النقود',
    refresh: 'تحديث',
    revenue: 'الإيرادات',
    expenses: 'المصروفات',
    totalCustomerDebt: 'إجمالي ديون الزبائن',
    customersWithDebt: 'زبون لديه ديون',
    recentTransactions: 'المعاملات الأخيرة',
    noTransactionsFound: 'لم يتم العثور على معاملات',
    tryAdjustingFilters: 'حاول تعديل المرشحات لرؤية المزيد من النتائج',
    noTransactionsAvailable: 'لا توجد معاملات متاحة في الوقت الحالي',
    clearAllFilters: 'مسح جميع المرشحات',
    showing: 'عرض',
    of: 'من',
    transactions: 'معاملة',
    filteredBy: 'مفلتر بواسطة',
    criteria: 'معايير',
    vsPrevPeriod: 'مقارنة بالفترة السابقة',
    search: 'البحث',
    type: 'النوع',
    currency: 'العملة',
    dateRange: 'التاريخ',
    amountRange: 'المبلغ'
  },
  losses: {
    title: 'خسائر المخزون',
    reportSpoilage: 'تسجيل تلف',
    reasons: {
      shrinkage: 'نقص وزن',
      spoiled: 'تالف / فاسد'
    },
    sources: {
      auto_close: 'تلقائي (إغلاق الفاتورة)',
      manual: 'يدوي'
    },
    lossDescription: 'خسارة مخزون ({{reason}}): {{amount}} {{product}}',
    quantityLost: 'الكمية المفقودة',
    notes: 'ملاحظات',
    notesPlaceholder: 'ملاحظة اختيارية (مثال: سقط صندوق أثناء النقل)',
    lossValue: 'قيمة الخسارة',
    onHand: 'المتوفر',
    confirmLoss: 'تسجيل الخسارة',
    recording: 'جارٍ التسجيل...',
    commissionNotice: 'بضاعة عمولة — تُسجَّل كملاحظة فقط؛ الخسارة على المورّد ولا تؤثر على حساباتك.',
    cannotExceedOnHand: 'لا يمكن تسجيل خسارة أكبر من الكمية المتوفرة',
    quantityRequired: 'أدخل كمية أكبر من صفر',
    lossRecorded: 'تم تسجيل الخسارة',
    lossRecordFailed: 'فشل تسجيل الخسارة',
    unaccountedUnits: 'وحدات غير مبررة',
    autoClassifyNotice: 'ستُسجَّل هذه الوحدات تلقائيًا كتالفة عند إغلاق هذه الفاتورة',
    shrinkagePreview: 'نقص الوزن الذي سيُسجَّل عند الإغلاق',
    estimatedLoss: 'الخسارة التقديرية',
    received: 'المستلم',
    sold: 'المباع',
    recordedLosses: 'الخسائر المسجلة',
    remainingWeight: 'الوزن المتبقي',
    weightRequired: 'الوزن مطلوب لهذا الصنف الموزون',
    weightTracked: 'تتبع الوزن',
    weightTrackedHint: 'سيكون الوزن مطلوبًا في كل عملية بيع لهذا الصنف',
    reverse: 'عكس',
    reversed: 'معكوس',
    reversalOf: 'عكس لـ',
    confirmReverse: 'عكس هذه الخسارة؟ سيتم استرجاع المخزون وعكس القيد المحاسبي.',
    lossReversed: 'تم عكس الخسارة',
    lossReverseFailed: 'فشل عكس الخسارة',
    restore: 'استعادة',
    confirmRestore: 'استعادة هذه الخسارة؟ سيتم إرجاع المخزون وإزالة الخسارة.',
    lossRestored: 'تمت استعادة الخسارة',
    lossRestoreFailed: 'فشل استعادة الخسارة',
    auditSpoiled: 'تلف {{count}} قطعة',
    auditRestored: 'تمت استعادة {{count}} قطعة تالفة',
    lossHistory: 'سجل الخسائر',
    status: 'الحالة',
    active: 'نشط',
    date: 'التاريخ',
    product: 'المنتج',
    lot: 'الدفعة',
    reason: 'السبب',
    quantity: 'الكمية',
    weight: 'الوزن',
    value: 'القيمة',
    recordedBy: 'سجّلها',
    totalLosses: 'إجمالي الخسائر',
    byReason: 'حسب السبب',
    byProduct: 'حسب المنتج',
    bySupplier: 'حسب المورّد/الفاتورة',
    noLosses: 'لا توجد خسائر مسجلة',
    shrinkageAnomaly: 'الوزن المباع يتجاوز الوزن المستلم لهذه الدفعة — عُلّمت للمراجعة ولم يُسجَّل نقص وزن'
  },
  payments: {
    changingDirectionNote: '(الدفع ⇆ الاسترداد)',
    corrected: 'تم تصحيحها',
    superseded: 'تم استبدالها',
    original: 'الأصلية',
    creditSaleBill: 'فاتورة بيع دين',
    billCancellation: 'إلغاء الفاتورة',
    billCancellationRefund: 'إلغاء الفاتورة المستردة',
    billReactivation: 'إعادة تنشيط الفاتورة',
    reversalUpdatedAdvancePayment: 'إلغاء الدفعة المعدلة',
    advancePayment: 'دفعة مقدمة',
    supplierAdvance: 'دفعة مورد',
    recentPayments: 'المدفوعات الأخيرة'
    ,
    supplierPayment: 'دفعة مورد',
    customerPayment: 'دفعة زبون',
    refund: 'دفعة مستردة',
    selectEntity: 'اختر الجهة...',
    pleaseSelectEntity: 'يرجى اختيار الجهة',
    changingEntityNote: 'تغيير الجهة يعكس العملية الأصلية على الجهة السابقة ويسجّل التصحيح على الجهة المختارة.',
    cannotCorrectNonActive: 'تم تصحيح هذه الدفعة أو عكسها بالفعل ولا يمكن تعديلها بعد الآن.',
    cannotDeleteNonActive: 'هذه الدفعة لم تعد نشطة ولا يمكن حذفها.',
    completed: 'مكتمل',
    reversed: 'معدل',
    canceled: 'ملغاة',
    paymentCanceled: 'تم إلغاء الدفعة',
    reversal: 'إلغاء',
    showCorrectedReversedPayments: 'عرض المدفوعات المعدلة ',
    title: 'إدارة المدفوعات',
    subtitle: 'تتبع وإدارة جميع معاملات الدفع',
    addPayment: 'إضافة دفعة',
    paymentsReceived: 'المدفوعات المستلمة',
    paymentsMade: 'المدفوعات المدفوعة',
    totalPayments: 'إجمالي المدفوعات',
    netAmount: 'المبلغ الصافي',
    netPositive: 'صافي موجب',
    netNegative: 'صافي سالب',
    totalTransactions: 'إجمالي المعاملات',
    allPaymentActivities: 'جميع أنشطة الدفع',
    filtersAndSearch: 'المرشحات والبحث',
    search: 'البحث',
    searchPlaceholder: 'البحث في الأوصاف والمراجع والكيانات...',
    dateRange: 'نطاق التاريخ',
    category: 'الفئة',
    allCategories: 'جميع الفئات',
    entityType: 'نوع العميل',
    allEntities: 'جميع العملاء',
    customers: 'الزبائن',
    suppliers: 'الموردين',
    customer: 'زبون',
    supplier: 'مورد',
    allCustomers: 'جميع الزبائن',
    allSuppliers: 'جميع الموردين',
    direction: 'الاتجاه',
    allDirections: 'جميع الاتجاهات',
    received: 'مستلم',
    paid: 'مدفوع',
    currency: 'العملة',
    allCurrencies: 'جميع العملات',
    amountRange: 'نطاق المبلغ',
    min: 'الحد الأدنى',
    max: 'الحد الأقصى',
    clearAllFilters: 'مسح جميع المرشحات',
    editPayment: 'تعديل الدفعة',
    paymentReceived: 'دفعة مستلمة',
    paymentMade: 'دفعة مدفوعة',
    unknown: 'غير معروف',
    amount: 'المبلغ',
    description: 'الوصف',
    descriptionPlaceholder: 'وصف الدفعة...',
    reference: 'المرجع',
    referencePlaceholder: 'رقم المرجع أو الملاحظة...',
    relatedEntity: 'العميل المرتبط',
    selectCustomer: 'اختر زبون...',
    selectSupplier: 'اختر مورد...',
    cancel: 'إلغاء',
    saveChanges: 'حفظ التغييرات',
    saving: 'جاري الحفظ...',
    amountRequired: 'يجب أن يكون المبلغ أكبر من 0',
    descriptionRequired: 'الوصف مطلوب',
    entityRequired: 'يجب اختيار زبون أو مورد',
    bothEntitiesError: 'لا يمكن اختيار الزبون والمورد معاً',
    updateFailed: 'فشل في تحديث الدفعة. يرجى المحاولة مرة أخرى.',
    paymentTransactions: 'معاملات الدفع',
    showingResults: 'عرض {{current}} من {{total}} معاملة',
    exportCSV: 'تصدير CSV',
    dateTime: 'التاريخ والوقت',
    entity: 'العميل',
    actions: 'الإجراءات',
    viewDetails: 'عرض التفاصيل',
    deletePayment: 'حذف الدفعة',
    noPaymentsFound: 'لم يتم العثور على مدفوعات',
    noPaymentsMessage: 'لا توجد معاملات دفع تطابق المرشحات الحالية.',
    previous: 'السابق',
    next: 'التالي',
    page: 'صفحة',
    of: 'من',
    deletePaymentTitle: 'حذف الدفعة',
    deletePaymentMessage: 'هل أنت متأكد من حذف هذه الدفعة؟ لا يمكن التراجع عن هذا الإجراء وسيؤثر على الأرصدة المرتبطة.',
    loadingDeletionDetails: 'جاري تحميل تفاصيل الحذف...',
    deletionBalanceImpact: 'تأثير الرصيد',
    currentBalance: 'الرصيد الحالي',
    balanceAfterDeletion: 'الرصيد بعد الحذف',
    balanceChange: 'التغيير',
    deletionSyncStatusSynced: '⚠️ تمت مزامنة هذه الدفعة مع الخادم',
    deletionSyncStatusUnsynced: 'ℹ️ لم يتم مزامنة هذه الدفعة بعد',
    deletionCashDrawerImpact: '💵 سيؤثر هذا الحذف أيضًا على رصيد درج النقد',
    deletionHasReversals: '⚠️ هذه الدفعة لها معاملات إلغاء مرتبطة',
    deletionWarning: 'تحذيرات',
    paymentDetails: 'تفاصيل الدفعة',
    balanceImpact: 'تأثير الرصيد',
    cashDrawer: 'درج النقود:',
    willBeReversed: 'سيتم عكسه',
    customerBalance: 'رصيد الزبون:',
    supplierBalance: 'رصيد المورد:',
    balanceWillBeAdjusted: 'سيتم تعديل الرصيد',
    noBalanceImpacts: 'لم يتم اكتشاف تأثيرات على الأرصدة لهذه الدفعة.',
    deletePaymentButton: 'حذف الدفعة',
    paymentDetailsTitle: 'تفاصيل الدفعة',
    transactionInfo: 'معلومات المعاملة',
    entityInfo: 'معلومات العميل',
    entityName: 'اسم العميل',
    noEntity: 'لا يوجد عميل',
    unknownCustomer: 'زبون غير معروف',
    unknownSupplier: 'مورد غير معروف',
    close: 'إغلاق',
    paymentUpdatedSuccessfully: 'تم تحديث الدفعة بنجاح',
    paymentDeletedSuccessfully: 'تم إلغاء الدفعة بنجاح',
    failedToUpdatePayment: 'فشل في تحديث الدفعة: {{error}}',
    failedToDeletePayment: 'فشل في حذف الدفعة: {{error}}',
    cashDrawerUpdated: 'درج النقود: {{previous}} → {{new}}',
    entityBalanceUpdated: 'تم تحديث رصيد {{entityType}}',
    // Payment Categories
    customerCreditSale: 'بيع بالدين للعميل',
    creditSaleDescription: 'فاتورة بيع دين - {{billNumber}}',
    creditSaleDescriptionCustomer: 'فاتورة بيع دين - {{billNumber}}',
    creditSaleDescriptionSupplier: 'فاتورة بيع دين - {{billNumber}}',
    paymentReceivedFrom: 'دفعة مستلمة من {{entityName}}',
    paymentSentTo: 'دفعة ل {{entityName}}',
    employeePayment: 'دفعة موظف',
    supplierCommission: 'عمولة مورد',
    cashPayment: 'دفعة نقدية',
    cashSale: 'بيع نقدي',
    paymentSent: 'دفعة مرسلة',
    expensePayment: 'دفعة مصروف',
    // Undo functionality
    undo: 'تراجع',
    undoLastAction: 'التراجع عن آخر إجراء',
    undoSuccessful: 'تم التراجع عن الإجراء بنجاح',
    undoFailed: 'لا يمكن التراجع: تم مزامنة الإجراء أو لم تعد السجلات موجودة',
    undoError: 'فشل التراجع عن الإجراء: {{error}}',
    // تحذيرات تغيير العميل
    warning: 'تحذير',
    entityChangeWarning: 'أنت تقوم بتغيير الدفعة من {{oldEntity}} إلى {{newEntity}}',
    entityChangeNote: 'سيؤدي هذا إلى إزالة الدفعة من حساب العميل السابق وإضافتها إلى حساب العميل الجديد. سيتم تحديث كلا الرصيدين وفقًا لذلك.',
    categoryCannotBeChanged: 'لا يمكن تغيير فئة الدفع بعد الإنشاء'
  },
  nonPriced: {
    title: 'العناصر غير المسعرة',
    exportCSV: 'تصدير CSV',
    allStagedChangesCleared: 'تم مسح جميع التغييرات المؤقتة',
    clearAllChanges: 'مسح جميع التغييرات',
    bulkActions: 'إجراءات مجمعة ({{count}})',
    itemsSelected: 'تم اختيار {{count}} عنصر',
    markAsPriced: 'وضع سعر',
    delete: 'حذف',
    clearSelection: 'مسح الاختيار',
    stagedChangesCleared: 'تم مسح التغييرات المؤقتة',
    clearChanges: 'مسح التغييرات',
    searchPlaceholder: 'البحث بالعميل أو المنتج أو المورد أو الحالة...',
    sortByDate: 'التاريخ',
    sortByCustomer: 'الزبون',
    sortByProduct: 'المنتج',
    sortBySupplier: 'المورد',
    sortByValue: 'القيمة',
    allTab: 'الكل',
    soldBillsTab: 'الفواتير المباعة',
    receivedBillsTab: 'الفواتير المستلمة',
    unitCost: 'سعر التكلفة',
    totalCost: 'التكلفة الإجمالية',
    searchPlaceholderSold: 'البحث بالعميل أو المنتج أو الحالة...',
    searchPlaceholderReceived: 'البحث بالمورد أو المنتج أو الحالة...',
    noSoldItems: 'لا توجد عناصر مباعة غير مسعرة',
    noReceivedItems: 'لا توجد عناصر مستلمة غير مسعرة',
    status: 'الحالة',
    customer: 'الزبون',
    product: 'المنتج',
    supplier: 'المورد',
    quantity: 'الكمية',
    weight: 'الوزن (كغ)',
    unitPrice: 'سعر الوحدة',
    totalValue: 'القيمة الإجمالية',
    dateAdded: 'تاريخ الإضافة',
    actions: 'الإجراءات',
    noItemsFound: 'لم يتم العثور على عناصر غير مسعرة',
    noItemsMessage: 'ستظهر العناصر هنا عندما تحتاج إلى تسعير.',
    ready: 'جاهز',
    incomplete: 'غير مكتمل',
    modified: 'معدل',
    editDetails: 'تعديل التفاصيل',
    deleteItem: 'حذف العنصر',
    showingResults: 'عرض {{start}} إلى {{end}} من {{total}} عنصر',
    previous: 'السابق',
    next: 'التالي',
    page: 'صفحة',
    of: 'من',
    reminder: 'يوجد {{count}} عنصر غير مسعر، يرجى تسعيرها.',
    goPrice: 'الذهاب إلى التسعير'
  },
  soldBills: {
    title: 'الفواتير المباعة',
    subtitle: 'مراجعة وإدارة الفواتير المباعة',
    loadingFinancialOperations: 'جاري تحميل العمليات المالية...',
    syncing: 'جاري المزامنة...',
    synced: 'تمت المزامنة',
    syncError: 'خطأ في المزامنة',
    offline: 'غير متصل',
    export: 'تصدير',
    refresh: 'تحديث',
    sync: 'مزامنة',
    syncCompletedSuccessfully: 'تمت المزامنة بنجاح',
    syncFailed: 'فشلت المزامنة',
    searchPlaceholder: 'البحث في الفواتير بالرقم أو الزبون أو الملاحظات...',
    dateFrom: 'من تاريخ',
    dateTo: 'إلى تاريخ',
    paymentStatus: 'حالة الدفع',
    billStatus: 'حالة الفاتورة',
    allPaymentStatus: 'جميع حالات الدفع',
    allStatus: 'جميع الحالات',
    paid: 'مدفوعة',
    partial: 'جزئية',
    pending: 'معلقة',
    active: 'نشطة',
    cancelled: 'ملغية',
    refunded: 'مستردة',
    billDetails: 'تفاصيل الفاتورة',
    customer: 'الزبون',
    amount: 'المبلغ',
    payment: 'الدفع',
    status: 'الحالة',
    actions: 'الإجراءات',
    noBillsFound: 'لم يتم العثور على فواتير',
    noBillsMessage: 'ستظهر الفواتير المنشأة من نقطة البيع هنا',
    createdBy: 'أنشأها',
    unknown: 'غير معروف',
    due: 'مستحق',
    viewDetails: 'عرض التفاصيل',
    editBill: 'تعديل الفاتورة',
    cancelBill: 'إلغاء الفاتورة',
    viewAuditTrail: 'عرض سجل التدقيق',
    billInformation: 'معلومات الفاتورة',
    billNumber: 'رقم الفاتورة',
    date: 'التاريخ',
    paymentMethod: 'طريقة الدفع',
    paymentInformation: 'معلومات الدفع',
    subtotal: 'المجموع الفرعي',
    total: 'المجموع',
    amountPaid: 'المبلغ المدفوع',
    amountDue: 'المبلغ المستحق',
    lineItems: 'عناصر السطر',
    product: 'المنتج',
    supplier: 'المورد',
    quantity: 'الكمية',
    price: 'السعر',
    weight: 'الوزن',
    notes: 'ملاحظات',
    items: 'عناصر',
    item: 'عنصر',
    inThisBill: 'في هذه الفاتورة',
    showLineItems: 'إظهار عناصر السطر',
    hideLineItems: 'إخفاء عناصر السطر',
    showDetails: 'إظهار التفاصيل',
    hideDetails: 'إخفاء التفاصيل',
    andMore: 'و',
    moreItems: 'عناصر إضافية',
    viewAll: 'عرض الكل',
    lineOrder: 'الترتيب',
    noLineItems: 'لا توجد عناصر سطر',
    walkInCustomer: 'زبون عابر',
    cash: 'نقدي',
    card: 'بطاقة',
    credit: 'دين',
    addNotesPlaceholder: 'إضافة ملاحظات حول هذه الفاتورة...',
    cancel: 'إلغاء',
    saving: 'جاري الحفظ...',
    saveChanges: 'حفظ التغييرات',
    auditTrail: 'سجل التدقيق',
    changedBy: 'تم التغيير بواسطة',
    unknownUser: 'مستخدم غير معروف',
    field: 'الحقل',
    reason: 'السبب',
    old: 'قديم',
    new: 'جديد',
    noAuditTrailAvailable: 'لا يوجد سجل تدقيق متاح لهذه الفاتورة'
  },
  receivedBills: {
    credit: 'دين',
    title: 'الفواتير المستلمة',
    exportCSV: 'تصدير CSV',
    problematicItemsWarning: '{{count}} عنصر مخزون لا يحتوي على received_quantity. انقر على "إصلاح البيانات" للتحقق، أو أضف عناصر مخزون جديدة لتتبع التقدم بشكل صحيح.',
    totalBills: 'إجمالي الفواتير',
    inProgress: 'قيد التقدم',
    completed: 'مكتملة',
    totalRevenue: 'إجمالي الإيرادات',
    search: 'البحث',
    searchPlaceholder: 'البحث في المنتجات والموردين...',
    product: 'المنتج',
    supplier: 'المورد',
    status: 'الحالة',
    statusAllStatus: 'جميع الحالات',
    statusNearlycomplete: 'تقريباً مكتمل',
    type: 'النوع',
    quantity: 'الكمية',
    progress: 'التقدم',
    actions: 'الإجراءات',
    allProducts: 'جميع المنتجات',
    allSuppliers: 'جميع الموردين',
    allStatus: 'جميع الحالات',
    allTypes: 'جميع الأنواع',
    pending: 'معلق',
    halfway: 'نصف الطريق',
    nearlyComplete: 'تقريباً مكتمل',
    closed: 'مغلق',
    statusPending: 'معلق',
    statusInProgress: 'قيد التقدم',
    statusHalfway: 'نصف الطريق',
    statusNearlyComplete: 'تقريباً مكتمل',
    statusCompleted: 'مكتمل',
    statusClosed: 'مغلق',
    commission: 'عمولة',
    cash: 'نقدي',
    mixed: 'مختلط',
    batch: 'دفعة',
    bill: 'فاتورة',
    original: 'الأصلية',
    remaining: 'المتبقي',
    revenue: 'الإيرادات',
    date: 'التاريخ',
    editBatch: 'تعديل الدفعة',
    details: 'التفاصيل',
    salesLogs: 'سجلات المبيعات',
    weightAnalysis: 'تحليل الوزن',
    item: 'عنصر',
    items: 'عناصر',
    inThis: 'في هذه',
    showingResults: 'عرض {{start}} إلى {{end}} من {{total}} نتيجة',
    previous: 'السابق',
    next: 'التالي',
    page: 'صفحة',
    of: 'من',
    receivedBillDetails: 'تفاصيل الفاتورة المستلمة',
    basicInformation: 'المعلومات الأساسية',
    receivedDate: 'تاريخ الاستلام',
    receivedBy: 'تم الاستلام بواسطة',
    quantityProgress: 'الكمية والتقدم',
    originalQuantity: 'الكمية الأصلية',
    remainingQuantity: 'الكمية المتبقية',
    soldQuantity: 'الكمية المباعة',
    totalReceivedWeight: 'إجمالي الوزن المستلم',
    totalSoldWeight: 'إجمالي الوزن المباع',
    financialInformation: 'المعلومات المالية',
    totalCost: 'إجمالي التكلفة',
    totalProfit: 'إجمالي الربح',
    commissionDetails: 'تفاصيل العمولة',
    porterage: 'رسوم العتالة',
    transferFee: 'رسوم النقل',
    commissionRate: 'معدل العمولة',
    averageUnitPrice: 'متوسط سعر الوحدة',
    notes: 'ملاحظات',
    viewSalesLogs: 'عرض سجلات المبيعات',
    close: 'إغلاق',
    totalSales: 'إجمالي المبيعات',
    avgPrice: 'متوسط السعر',
    totalPrice: 'السعر الإجمالي',
    paymentMethod: 'طريقة الدفع',
    editSale: 'تعديل البيع',
    deleteSale: 'حذف البيع',
    noSalesRecorded: 'لم يتم تسجيل مبيعات',
    noSalesMessage: 'لم يتم تسجيل أي مبيعات لهذا العنصر من المخزون بعد.',
    showingSales: 'عرض {{count}} سجل مبيعات{{count === 1 ? "" : "ات"}}',
    exportBill: 'تصدير الفاتورة',
    closeBill: 'إغلاق الفاتورة',
    exportOnlyAfterClosing: 'التصدير متاح فقط بعد إغلاق الفاتورة',
    billAlreadyClosed: 'الفاتورة مغلقة بالفعل',
    cannotCloseBill: 'لا يمكن إغلاق الفاتورة: كمية مفقودة أو عنصر غير مسعر موجود',
    cannotCloseBillWithNonPricedItems: 'لا يمكن إغلاق الفاتورة: {count} عنصر بدون سعر. يرجى تعيين الأسعار لـ: {products}',
    closeBillConfirmation: 'تأكيد إغلاق الفاتورة',
    billSummary: 'ملخص الفاتورة',
    feeBreakdown: 'تفصيل الرسوم',
    confirmCloseBill: 'تأكيد إغلاق الفاتورة',
  },
  cashDrawer: {
    currentStatus: 'الحالة الحالية',
    activeSession: 'جلسة نشطة',
    currentBalance: 'الرصيد الحالي',
    openedBy: 'فتح بواسطة',
    employee: 'الموظف',
    sessionDuration: 'مدة الجلسة',
    timeOpen: 'وقت الفتح',
    openedAt: 'فتح في',
    openingAmount: 'مبلغ الافتتاح',
    closingProcess: 'عملية إغلاق الصندوق',
    inventoryCheck: 'فحص المخزون',
    cashBalance: 'رصيد النقدية',
    closeDrawer: 'إغلاق الدرج',
    inventoryVerificationCompleted: 'تم إكمال فحص المخزون. يمكنك الإلغاء وإعادة البدء إذا لزم الأمر.',
    continueToCashBalance: 'المتابعة إلى رصيد النقدية',
    startClosingProcess: 'بدء عملية الإغلاق',
    refreshStatus: 'تحديث الحالة',
    noActiveSession: 'لا توجد جلسة نشطة',
    closedStatus: 'الصندوق مغلق',
    mustBeOpenedMessage: 'يجب فتح الصندوق لبدء تتبع المعاملات',
    verifyCashBalance: 'التحقق من رصيد النقدية',
    expectedAmount: 'المبلغ المتوقع',
    countCashInstructions: 'عد النقد الفعلي في الدرج وأدخل المبلغ أدناه.',
    actualAmountInDrawer: 'المبلغ الفعلي في الدرج',
    enterActualAmount: 'أدخل المبلغ الفعلي',
    validAmountRequired: 'يرجى إدخال مبلغ صحيح أكبر من 0',
    variance: 'الفرق',
    over: 'زيادة',
    short: 'نقص',
    cancel: 'إلغاء',
    closing: 'جاري الإغلاق...',
  },
  balanceReport: {
    financialSummary: 'ملخص مالي',
    detailedView: 'عرض مفصل',
    totalCreditSales: 'إجمالي المبيعات بالدين',
    pagesSelected: 'الصفحات المختارة',
    number: 'العدد',
    balanceAfter: 'الرصيد بعد',
    previewDocumentBeforePrinting: 'معاينة المستند قبل الطباعة',
    selectAllPages: 'اختيار جميع الصفحات',
    selectPages: 'اختيار الصفحات',
    page: 'صفحة',
    contentNotAvailable: 'المحتوى غير متاح',
    print: 'طباعة',
    cancel: 'إلغاء',
    debitTotal: 'إجمالي المدين',
    creditTotal: 'إجمالي الدائن',
    debit: 'مدين',
    credit: 'دائن',
    balance: 'الرصيد',
    date: 'التاريخ',
    reference: 'المرجع',
    description: 'الوصف',
    quantity: 'الكمية',
    weight: 'الوزن',
    price: 'السعر',
    title: 'تقرير رصيد الصندوق',
    last30Days: 'آخر 30 يوم',
    startDate: 'تاريخ البداية',
    endDate: 'تاريخ النهاية',
    refresh: 'تحديث',
    exportCSV: 'تصدير CSV',
    totalSessions: 'إجمالي الجلسات',
    balanced: 'متوازن',
    unbalanced: 'غير متوازن',
    avgVariance: 'متوسط الفرق',
    clickRowForDetails: 'انقر على أي صف لعرض تفاصيل المعاملات',
    employee: 'الموظف',
    opening: 'الافتتاح',
    expected: 'المتوقع',
    actual: 'الفعلية',
    variance: 'الفرق',
    status: 'الحالة',
    notes: 'ملاحظات',
    noSessionsFound: 'لم يتم العثور على جلسات الصندوق',
    noSessionsMessage: 'لم يتم العثور على جلسات صندوق مغلقة للفترة المحددة. يتم إنشاء جلسات الصندوق عندما يفتح الموظفون ويغلقون الصندوق.',
    sessionDetails: 'تفاصيل الجلسة',
    sessionSummary: 'ملخص الجلسة',
    openingAmount: 'مبلغ الافتتاح',
    expectedAmount: 'المبلغ المتوقع',
    actualAmount: 'المبلغ الفعلي',
    transactionSummary: 'ملخص المعاملات',
    cashSales: 'مبيعات نقدية',
    cashPayments: 'مدفوعات نقدية',
    cashExpenses: 'مصروفات نقدية',
    transactions: 'معاملات',
    loadingTransactionDetails: 'جاري تحميل تفاصيل المعاملات...',
    transactionDetails: 'تفاصيل المعاملات',
    product: 'المنتج',
    unitPrice: 'سعر الوحدة',
    total: 'الإجمالي',
    unknown: 'غير معروف',
    amount: 'المبلغ',
    category: 'الفئة',
    account: 'الحساب',
    netPosition: 'الرصيد الصافي',
    owesUs: 'يدين لنا',
    owedToEntity: 'مستحق لـ',
    multipleCurrencies: 'عملات متعددة',
    accountStream: 'تيار الحساب',
    noConversionNote: 'الأرصدة معروضة لكل عملة (بدون تحويل صرف).'
  },
  receipt: {
    direction: 'rtl',
    storeName: 'سوق كيوي للخضار',
    address: '63-B2-سوق الجملة، طرابلس - لبنان',
    phone: 'الهاتف',
    phones: 'الهواتف',
    phone1Name: 'سمير',
    phone2Name: 'محمد',
    thankYouMessage: 'شكراً لكم!',
    billNumberPrefix: '٠٠٠',
    showPreviousBalance: true,
    showItemCount: true,
    receiptWidth: 32,
    accountName: 'اسم الحساب',
    accountNumber: 'رقم الحساب',
    fromDate: 'من تاريخ',
    toDate: 'إلى تاريخ',
    pageNumber: 'رقم الصفحة',
    billNumber: 'رقم الفاتورة',
    date: 'التاريخ',
    customer: 'الزبون',
    subtotal: 'المجموع الفرعي',
    total: 'الإجمالي',
    totalItems: 'عدد الأصناف',
    previousBalance: 'الرصيد السابق',
    unknownProduct: 'منتج غير معروف',
    itemHeader: 'الصنف        الكمية  الوزن  السعر  المجموع',
    lineSeparator: '=',
    dashSeparator: '-',
    lbp: 'ل.ل',
    totalBalance: 'الرصيد الإجمالي',
    scanQRCode: 'امسح الكود الخاص بك للحصول على كشف الحساب',
    currency: 'ل.ل'
  },
  unsyncedItems: {
    title: 'العناصر غير المتزامنة',
    subtitle: 'عناصر في انتظار المزامنة مع الخادم',
    totalUnsynced: 'إجمالي غير المتزامنة',
    tablesAffected: 'الجداول المتأثرة',
    lastRefresh: 'آخر تحديث',
    allSynced: 'جميع العناصر متزامنة',
    noPendingItems: 'لا توجد عناصر في انتظار المزامنة.',
    offlineWarning: 'أنت غير متصل حاليًا. سيتم مزامنة العناصر عند استعادة الاتصال.'
  },
  bill: {
    settlementPickerLabel: 'عملة الفاتورة',
    settlementNotAccepted: 'هذه العملة غير مقبولة لهذا المتجر.',
    conversionRateMissing: 'لا يوجد سعر صرف من {{from}} إلى {{to}}. حدّث الأسعار في الإعدادات.',
    currencyLocked: 'تُقفل العملة بعد أول بند في السلة.',
  },
  transaction: {
    currencyMissing: 'عملة المعاملة مطلوبة.',
    currencyNotAccepted: 'هذه العملة غير مقبولة لهذا المتجر.',
  },
  errors: {
    transaction: {
      amount_required: 'المبلغ مطلوب.',
      amount_non_positive: 'يجب أن يكون المبلغ أكبر من الصفر.',
      currency_invalid: 'رمز العملة غير معروف.',
      category_invalid: 'فئة المعاملة غير صالحة.',
      description_required: 'الوصف مطلوب.',
      store_id_required: 'معلومات المتجر مفقودة. يرجى إعادة تحميل التطبيق.',
      branch_id_required: 'معلومات الفرع مفقودة. يرجى إعادة تحميل التطبيق.',
      entity_not_found: 'تعذر العثور على العميل أو المورد المحدد.',
      cash_drawer_missing: 'لم يتم العثور على حساب درج النقد. يرجى إعداد واحد قبل معالجة هذه المعاملة.',
    },
    journal: {
      branch_id_required: 'معلومات الفرع مفقودة من قيد اليومية هذا.',
      debit_credit_mismatch: 'يجب أن تكون المدينات والدائنات في قيد اليومية متساوية.',
      amount_non_positive: 'يجب أن يكون مبلغ قيد اليومية أكبر من الصفر.',
      account_invalid: 'رمز حساب واحد أو أكثر في قيد اليومية هذا غير معروف.',
    },
    bill: {
      supplier_required: 'يجب تحديد مورد قبل حفظ هذه الفاتورة.',
      line_items_empty: 'يجب أن تحتوي هذه الفاتورة على بند واحد على الأقل.',
      amount_non_positive: 'يجب أن تكون مبالغ بنود الفاتورة أكبر من الصفر.',
    },
    statement: {
      token_invalid: 'رابط الكشف هذا غير صالح أو تم استخدامه بالفعل.',
      token_expired: 'انتهت صلاحية رابط الكشف هذا. يرجى طلب رابط جديد.',
    },
    system: {
      local_write_failed: 'تعذر حفظ التغييرات. يرجى المحاولة مرة أخرى.',
      sync_upload_failed: 'تعذر مزامنة التغييرات. ستتم إعادة المحاولة تلقائيًا.',
      network_unavailable: 'يبدو أنك غير متصل بالإنترنت. تتطلب هذه العملية اتصالًا.',
      unknown: 'حدث خطأ ما. يرجى المحاولة مرة أخرى أو إعادة تحميل التطبيق.',
    },
  },
  auditLog: {
    title: 'سجل التدقيق',
    subtitle: 'سجل لمن غيّر ماذا ومتى.',
    filters: 'عوامل التصفية',
    quickDateRange: 'نطاق زمني سريع',
    startDate: 'تاريخ البدء',
    endDate: 'تاريخ الانتهاء',
    preset: { today: 'اليوم', week: 'آخر 7 أيام', mtd: 'منذ بداية الشهر', month: 'آخر 30 يومًا', year: 'هذه السنة' },
    user: 'المستخدم',
    allUsers: 'كل المستخدمين',
    action: 'الإجراء',
    entityType: 'النوع',
    all: 'الكل',
    clearFilters: 'إعادة تعيين',
    columns: { time: 'الوقت', user: 'المسؤول', action: 'الإجراء', entity: 'السجل', reference: 'المرجع', details: 'التغييرات', reason: 'الملخص' },
    actions: {
      create: 'إنشاء', update: 'تعديل', delete: 'حذف', void: 'إلغاء',
      reactivate: 'إعادة تفعيل', archive: 'أرشفة', unarchive: 'إلغاء الأرشفة', open: 'فتح', close: 'إغلاق',
      login: 'تسجيل الدخول', logout: 'تسجيل الخروج',
    },
    entityTypes: {
      entity: 'عميل / مورّد', product: 'منتج', user: 'موظف', branch: 'فرع',
      bill: 'فاتورة بيع', payment: 'دفعة', cash_drawer_session: 'درج النقد', inventory_item: 'صنف مخزون',
      inventory_batch: 'دفعة مخزون', product_category: 'فئة', unit_of_measure: 'وحدة',
      store_settings: 'إعدادات المتجر', user_permission: 'صلاحية', auth: 'المصادقة',
    },
    billSummary: {
      created: 'تم إنشاء الفاتورة بعدد {{count}} صنف (الإجمالي **{{total}} {{currency}}**)',
    },
    inventorySummary: {
      received: 'تم استلام فاتورة المخزون ({{count}} صنف، {{type}})',
      types: { cash: 'نقدًا', credit: 'آجل', commission: 'عمولة' },
    },
    paymentSummary: {
      entityPayment: 'دفعة {{entityType}} {{direction}}: **{{amount}} {{currency}}** ({{name}})',
      entityTypes: { customer: 'عميل', supplier: 'مورّد' },
      directions: { received: 'مستلمة', sent: 'مرسلة' },
    },
    fieldChange: '{{field}}: {{old}} ← {{new}}',
    emptyValue: '(فارغ)',
    changeCount: 'تم تغيير {{count}} حقل',
    noLogs: 'لا يوجد نشاط للمرشحات المحددة.',
    loadMore: 'تحميل المزيد',
    showingCount: 'عرض {{count}} سجلًا',
    scope: {
      cashier: 'أنت تعرض نشاطك الخاص.',
      manager: 'أنت تعرض نشاط فرعك.',
      admin: 'النشاط في المتجر بالكامل.',
    },
    history: {
      title: 'سجل التغييرات',
      none: 'لا توجد تغييرات مسجلة لهذا السجل.',
      loading: 'جارٍ تحميل السجل…',
    },
    view: {
      events: 'الأحداث',
      people: 'المستخدمون',
      updates: 'التعديلات',
      removals: 'عمليات الحذف',
      today: 'اليوم',
      yesterday: 'أمس',
      eventCount: '{{count}} حدثًا',
      activeFilters: 'مُصفّى',
      search: 'ابحث عن مستخدم أو سجل أو تغيير…',
      emptyHint: 'جرّب توسيع النطاق الزمني أو إعادة تعيين عوامل التصفية.',
      noMatch: 'لا توجد سجلات مطابقة لبحثك.',
      referenceHint: 'عرض كل السجلات لهذا المرجع',
    },
  },
};

export default ar;


/*
Log out button somtimes doesn't work, it doesn't navigate the user to login page instantly, it required to reload 
pressing it multiple times logs: 
ℹ️ No active check-in found, skipping check-out
SupabaseAuthContext.tsx:681 ✅ Branch preference cleared for store: 81b168f5-2ce5-4c40-9ac5-86e239d1af83
OfflineDataContext.tsx:1528 👀 [FOCUS-SYNC] Window focused, checking if sync needed...
SupabaseAuthContext.tsx:668 ℹ️ No active check-in found, skipping check-out
SupabaseAuthContext.tsx:681 ✅ Branch preference cleared for store: 81b168f5-2ce5-4c40-9ac5-86e239d1af83
CashDrawerMonitor.tsx:33 branch 675443 5754911d-bfe2-4629-90a0-3b83f275f601
CashDrawerMonitor.tsx:36 💰 Balance: 0
SupabaseAuthContext.tsx:668 ℹ️ No active check-in found, skipping check-out
SupabaseAuthContext.tsx:681 ✅ Branch preference cleared for store: 81b168f5-2ce5-4c40-9ac5-86e239d1af83
**/