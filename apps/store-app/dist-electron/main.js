...
ipcMain.handle('print-formal-bill', async (_event, payload) => {
    try {
        ...
        const printerName = payload.printerName && payload.printerName !== ''
            ? payload.printerName
            : undefined;
        const printOptions = {
            silent: true,
...
function generateFormalBillHTML(payload) {
    const rs = payload.receiptSettings || {};
    const bill = payload.bill || {};
    const entity = payload.entity || null;
    const lineItems = payload.lineItems || [];
    const lang = payload.language || 'en';
    const currency = payload.currency || 'LBP';
    const exchangeRate = payload.exchangeRate || 0;
    const isRTL = lang === 'ar';
    function esc(s) {
        return String(s || '')
            .replace(/&/g, '&amp;')
